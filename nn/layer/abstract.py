...
class AbsLayer(IOperator, ILazyInitialization):
    ...
    def F(self, x: [float, ndarray, tuple] = None, state: ModelState = ModelState.Training) -> Union[float, ndarray]:
        ...
        if state != ModelState.Training:
            begin = time.time()
            output = self.__activation.do_forward(self.do_forward_predict(self.__ref_input))
            self.__val_forward_time += time.time() - begin
            return output
        else:
            begin = time.time()
            output = self.__activation.do_forward(self.do_forward_train(self.__ref_input))
