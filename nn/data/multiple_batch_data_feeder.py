...
class MultipleBatchDataFeeder(IDataFeeder):

    def __init__(self, x: ndarray, batch_size: int, max_batch_num: int = 1):
        self.__iter_x: ndarray = x
        self.__batch_size = min(batch_size * max_batch_num, len(x))
        self.__batches = len(x) // (self.__batch_size * max_batch_num)
        self.__iter = 0
